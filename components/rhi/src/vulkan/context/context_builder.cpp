...
void InstanceBuilder::enable_validation_layers(ContextBuilder &context_builder)
{
	...
	const auto all_layers_are_available = [&](const std::vector<const char *> &requested) -> bool {
		for (auto &layer : requested)
		{
			...
			if (!found)
			{
				LOGW(Vulkan, "Validation layer {} is not available", layer);
				return false;
			}
		}
		return true;
	...
	for (auto &validation_layers : validation_layer_priority_list)
	{
		if (all_layers_are_available(validation_layers))
		{
			...
			{
				instance_builder.request_layer(layer, InclusionMode::Required);
			}
			break;
		}

		LOGW(Vulkan, "Couldn't enable all validation layers (see log) - falling back");
	}
}
...
void InstanceBuilder::enable_default_logger(ContextBuilder &context_builder)
{
	...
	context_builder
	    .configure_instance()
	    .add_logger_callback(std::make_shared<LoggerCallbacks>(
	        LoggerCallbacks{[](LogLevel level, const char *msg) {
		        switch (level)
		        {
			        case LogLevel::Debug:
				        LOGD(Vulkan, msg);
...
void InstanceBuilder::build(Context &context)
{
	...
	if (!logger_callbacks.empty())
	{
		...
		{
			LOGI(Vulkan, "No debug extension available - loggers will not be used");
		}
	}
	...
	for (auto &required_extension : required_extensions)
	{
		...
		{
			LOG_CHECK(Vulkan, true, "Required extension is not available: {}", required_extension);
		}
	}
	...
	for (auto &required_layer : required_layers)
	{
		...
		{
			LOG_CHECK(Vulkan, true, "Required layer is not available: {}", required_layer);
		}
	}
...
void DeviceBuilder::build(Context &context)
{
	...
	for (auto &extension_name : required_extensions)
	{
		...
		{
			LOG_CHECK(Vulkan, true, "Required extension is not available: {}", extension_name);
		}
	}
	...
	for (auto &layer_name : required_layers)
	{
		...
		{
			LOG_CHECK(Vulkan, true, "Required layer is not available: {}", layer_name);
		}
	}
...
#define FEATURE_CHECK(feature) LOG_CHECK(Vulkan, enabled_features.feature && !supported_features.feature, #feature " is not supported")
	...
	std::vector<Context::QueueGroup>       queue_groups;

	std::vector<float> queue_priorities;
	queue_priorities.resize(100, 1.0f);

	for (uint32_t queue_family_index = 0U; queue_family_index < queue_family_properties.size(); queue_family_index++)
	{
		auto &family = queue_family_properties[queue_family_index];
		...
		std::vector<vk::SurfaceKHR> supported_surfices;

		Context::QueueGroup group{};
		group.queue_family_index              = queue_family_index;
		group.supported_queues                = family.queueFlags;
		group.supported_presentation_surfaces = supported_surfices;
		group.queues                          = std::vector<vk::Queue>{family.queueCount, VK_NULL_HANDLE};
		queue_groups.push_back(group);
	}
	...
	context.queue_groups = std::move(queue_groups);
}
